(function(){
  ...
  function card(item){
    ...
    const emergencyPhones=(item.emergencyPhones||[]).map(phone=>`<a class="contact-phone" href="${phoneHref(phone)}"><span>${escapeHtml(item.emergencyPhoneLabel||'Emergency')}</span>${escapeHtml(phone)}</a>`).join('');
    const email=item.email?`<a class="contact-phone" href="mailto:${escapeHtml(item.email)}"><span>Email</span>${escapeHtml(item.email)}</a>`:'';
  ...
  function render(){
    ...
    const filtered=data.filter(item=>(category==='All'||item.category===category)&&(scopeValue==='All'||item.scope===scopeValue)&&(!q||[item.title,item.category,item.type,item.description,(item.services||[]).join(' '),(item.phones||[]).join(' '),(item.searchTerms||[]).join(' '),item.email||'',item.address||'',item.officeHours||'',(item.emergencyPhones||[]).join(' ')].join(' ').toLowerCase().includes(q))).sort((a,b)=>(a.priority||99)-(b.priority||99)||a.title.localeCompare(b.title));
